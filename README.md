...
notch [flags] [prompt words...]
  ...
  --max-turns int         maximum model turns per run (0 is unlimited)
  --max-cost-usd float    maximum cumulative cost in USD (0 is unlimited)
  --idle-timeout duration cancel after no provider or tool events (for example 2m)
  --setting-sources list  user, project, user,project (default), or none
...
`--mode rpc` exposes asynchronous Pi-style command responses and streaming events over strict JSONL; see [RPC mode](docs/rpc.md). Tool flags apply to every run mode after built-in, extension, and MCP registration. JSONL runs emit `session_start` with the resolved model, tools, skills, instructions, and workspace trust state, then a terminal `run_end` with cumulative usage, cost accounting status, turns, elapsed time, final text, and stop reason. Reaching `--max-turns` or `--max-cost-usd` exits with status 2; clean runs exit 0 and errors exit 1. Unknown pricing is explicit and prevents enforcement of a requested cost ceiling.
...
`--continue` selects the latest session whose original working directory matches the current working directory. Bare `--resume` opens a compact, in-place picker for the five most recently active sessions from the current working directory; use Up/Down and Enter, or type to filter the first-prompt preview. `--resume ID-OR-PREFIX` opens a specific session by ID, filename, unambiguous prefix, or path. Fullscreen `/resume` presents saved sessions with time, original directory, model, and prompt preview. Resumed requests use the current provider/model configuration while preserving the selected session's conversation context. Each completed provider response, including compaction summaries, appends a `usage` record with provider, model, uncached input, output, cache-read, cache-write, and reasoning token counts. Usage records retain provider-reported cost when available, a versioned API-list-price estimate for known direct-provider models, the effective cost, its source, and the stop reason.

## Efficient tool defaults

- `read` defaults to 200 lines. Truncated pages include the next `offset`; set `limit` explicitly for larger reads.
- Recursive `grep` skips nested `.git` directories and files with NUL bytes in their first 8 KiB. Use `includeGit` or `includeBinary` to opt in; an explicit `.git` path is searched without `includeGit`.
- `list_monitors` defaults to active summaries. Set `status: "all"` for retained completed monitors, or `id` for one monitor. `includeOutput: true` requires `id`. Pass a returned `Snapshot` timestamp as `since` to see only changed statuses/output; this is not a durable change log and excludes pruned monitors. Completion notifications resume the agent automatically—do not poll. Command monitors watch process exits and output writes without an idle polling timer.
- Repeated `skill` loads return an acknowledgment only while the identical full instructions remain in the model's current context. Truncation, compaction, reset, or context-hook removal allows a full reload. Session history is unchanged.
- Read-only subagents support `read,grep,find,ls`. `bash` is write-capable even when intended for inspection; keep shell analysis in the parent unless the user explicitly authorizes delegated write access.

## Workspace trust

Project-controlled `.notch` and `.agents` inputs, plus root-level `AGENTS.md` and `AGENTS.local.md`, are loaded only for a trusted workspace. Notch loads project inputs from the active worktree's canonical Git root, while trust is keyed by the repository's canonical Git common directory, so one decision applies to the primary checkout and all linked worktrees. Outside Git, the canonical current directory is used for both. If supported project inputs exist and the repository is not already trusted, a run with terminal stdin and stdout prompts once and persists an accepted decision in `$XDG_DATA_HOME/notch/trusted-workspaces.json` (default `~/.local/share/notch/trusted-workspaces.json`). Runs without project inputs do not prompt.

Noninteractive runs never prompt and skip untrusted project config, MCP, extensions, resources, themes, and AGENTS instructions unless trust was previously persisted. Use `--trust-workspace` to persist trust explicitly for automation or CI. Use `--safe` to bypass project trust and project inputs for that invocation; it cannot be combined with `--trust-workspace`. Global inputs and installed extension packages remain available. For reproducible automation, `--setting-sources project` excludes user configuration, extensions, skills, prompts, themes, and installed extension packages while retaining trusted project inputs; `none` excludes both user and project sources. Trust is an execution boundary, not an approval workflow: Notch has no per-command approvals, and tools/extensions execute automatically after loading.

For a trusted workspace, Notch appends `<workspace-root>/AGENTS.md` to the configured system prompt when present, followed by `<workspace-root>/AGENTS.local.md`. The local file therefore has final instruction precedence and can hold machine-specific guidance without changing the shared file. Instruction discovery is intentionally root-scoped; Notch does not walk parent or child directories. The combined files are limited to 1 MiB.
...
3. `NOTCH_PROVIDER`, `NOTCH_MODEL`, `NOTCH_EXPLORE_MODEL`, and `NOTCH_THINKING_LEVEL`
4. CLI overrides such as `--provider`, `--model`, and `--thinking`

Later non-empty values replace earlier ones, so CLI flags take precedence over environment variables and environment variables take precedence over project and user config. Empty or whitespace-only values for the four runtime environment variables are ignored and fall back to the merged config files. Non-empty directory arrays replace the complete earlier array; they are not appended. `base_url` and `auto_update` are global-only and ignored in project config. `auth_file`, `mcp_auth_file`, `session_dir`, and `model_cache` are not JSON settings: they are fixed below the XDG data root and JSON keys with those names are ignored. API keys can come from environment variables; OAuth credentials are kept separately from config in the protected auth store. Standalone `notch login`, `logout`, `auth`, and `models` commands also load global configuration only.
...
```sh
NOTCH_PROVIDER=openai-codex NOTCH_MODEL=gpt-5.6-sol NOTCH_EXPLORE_MODEL=openai-codex/gpt-5.4-mini NOTCH_THINKING_LEVEL=high notch
```

When the optional [`rtk`](https://github.com/rtk-ai/rtk) executable is on `PATH` at startup, Notch's official RTK extension automatically rewrites supported `bash` calls through `rtk rewrite` so command output is filtered before entering model context. RTK is not bundled: if it is absent, Notch shows a startup warning and leaves bash unchanged; if RTK declines a command or cannot rewrite it, Notch silently runs the original command. RTK's own configuration controls command exclusions and output recovery. Use `--no-extensions` to disable the integration for a Notch invocation.

Exploration guidance is enabled by default—no experimental flag is needed. For broad discovery, `explore_codebase` asks workers for evidence-first reports (normally at most 500 words) with file/line references, relevant test evidence, and uncertainties. The parent is guided to verify critical claims with targeted reads rather than repeat the entire search; focused lookups stay direct. This is prompt guidance, not enforced routing or a guarantee of cost savings. Tool permissions are unchanged.

Provider, parent model, and explore model overrides are independent: unset values come from the merged config. `explore_model` selects the default child model for `explore_codebase`; use a provider-qualified value such as `openai-codex/gpt-5.4-mini` only when that cross-provider delegation is intentional. An explicit tool-call model still wins, while an empty or omitted tool model uses `explore_model`, then falls back to the current parent provider/model when `explore_model` is unset. Unqualified tool-call and explore model IDs are resolved on the currently selected parent provider, including after an in-session model switch. Per-task overrides win over the batch default. Notch instructs the agent not to cross providers silently: another provider should be used only when the user explicitly requests it or configures a provider-qualified delegation model. If a requested model is unavailable on the current provider and no alternate provider was named, the agent should ask before trying one. If an explore model is rejected as unavailable, the tool tells the parent to call `list_models` for the selected provider and retry once with a returned model in the closest family/capability tier rather than inventing an ID. In the fullscreen TUI, `presets` assigns provider/model/thinking combinations to `f1` through `f9`; pressing a configured function key changes subsequent turns without rewriting config. A preset may omit provider, model, or thinking level to preserve that current value. Global and trusted project preset maps merge by key, with project entries replacing matching global entries. `notch models [provider]` lists provider-discovered or fallback models; add `--refresh` to bypass the cache, `--json` for a versioned machine-readable catalog, or `--all` for every supported provider. Fullscreen `/model` selects a provider and filters its models, while `/model refresh` forces discovery. Runtime selection changes subsequent turns and new sessions but does not edit config files. The paths above illustrate the defaults; actual home and workspace-root paths are resolved at startup. `XDG_CONFIG_HOME` relocates config and user resources; `XDG_DATA_HOME` relocates private runtime data. Both must be absolute when set. `NOTCH_HOME` is ignored. Trusted project resources remain under `<workspace-root>/.notch`. Only global configured resource directories are created before trust; trusted project directories may then be created. Notch also discovers shared skills in `~/.agents/skills` and trusted `<workspace-root>/.agents/skills`, plus command templates in `~/.agents/commands` and trusted `<workspace-root>/.agents/commands`; these shared directories are discovered but never created by Notch. `theme` and `thinking_level` can be set in either global or trusted project config. Direct JSON files in `theme_dirs` add or override themes; see [themes](docs/themes.md) for the small semantic schema. The mode-0600 model cache refreshes stale selected-provider data on startup or selector use without a polling timer; `model_refresh_hours` defaults to 24. Interactive release builds also install a newer verified stable release at most once per day by default and relaunch immediately before opening a session; set global `auto_update` to `false` to opt out. See [releases and upgrades](docs/releases.md). `context_window: 0` means use the provider/model default; omit it for the same behavior. The compaction object deliberately uses Pi-compatible camelCase keys. See [themes](docs/themes.md), [thinking controls](docs/tui.md#commands-and-thinking-level), and [compaction](docs/compaction.md).

`cache_retention` accepts `none`, `short`, or `long` and defaults to `short`. Short caching uses provider-default retention; long caching requests one-hour Anthropic or 24-hour OpenAI retention where supported. Normal turns share a stable session affinity key, while one-off compaction summaries disable cache writes. Cache writes can cost more than ordinary input, so use `none` for retention-sensitive workloads or sessions unlikely to reuse a prompt prefix.
...
MCP schemas are discovered on demand. The model initially sees a compact `mcp_search` tool instead of every remote schema. It searches server/tool names and descriptions using case-insensitive keywords (all words must match), revealing up to three matching tools by default, or five with `limit`. Exact tool-name matches rank first, then matches in tool names, then description-only matches; ties are alphabetical. Revealed schemas appear in the next model request; calls use their original `mcp__<server>__<tool>` names through the normal execution, hook, and logging path. Discovery does not execute a remote tool.

For example, `mcp_search({"query":"sentry issues"})` reveals matching issue tools. Refine broad queries to avoid loading unnecessary schemas. Frequently used tools can be pinned by adding `"directTools": ["search_issues"]` to that server's MCP configuration (use exact **remote**, unprefixed tool names). `"directTools": ["*"]` restores eager schema exposure for that server. Unavailable pinned names are configuration errors.

This is lazy **schema disclosure**, not lazy server startup: connections and `tools/list` still happen at startup. Revealed tools remain exposed for the registration's lifetime; restarting or reloading MCP resets disclosure to configured pins. Resuming a session in a new process may require searching again. `/tools` and RPC `get_tools` retain the inventory of permitted tools, including deferred tools; JSONL `session_start.tools` lists schemas currently advertised to the model.

Tool permissions are unchanged. `--exclude-tools` removes targets from both discovery and execution. Explicit `--tools` and active-tool selections expose the selected tools directly; adding only `mcp_search` to a strict allowlist does **not** grant access to excluded targets. Disabling extensions does not disable MCP discovery, but `--no-tools` does. Plan mode keeps MCP search and calls disabled. Hiding a schema is not an access-control boundary; a known permitted tool name can still be called directly. Actual savings depend on discovery overhead, tool use, and caching.

Remote tools are registered as `mcp__<server>__<tool>`. Notch performs the MCP 2025-06-18 handshake, follows paginated `tools/list`, and calls tools over stdio or HTTP responses in JSON or SSE form. Resource/prompt MCP capabilities are not implemented yet.
