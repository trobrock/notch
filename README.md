...
MCP schemas are discovered on demand. The model initially sees a compact `mcp_search` tool instead of every remote schema. It searches server/tool names and descriptions using case-insensitive keywords (all words must match), revealing up to three matching tools by default, or five with `limit`. Revealed schemas appear in the next model request; calls use their original `mcp__<server>__<tool>` names through the normal execution, hook, and logging path. Discovery does not execute a remote tool.
