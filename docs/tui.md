...
- tool calls/results are full-width cards whose background reflects pending, successful, or failed status; long-running official delegation tools keep one live progress line with elapsed time, completed child turns/tool calls, and the last child tool, replacing that status instead of accumulating heartbeat lines;
