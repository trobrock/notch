...
MCP tool schemas are deferred by default: the model uses `mcp_search` with concise keywords to reveal a few matching tools, then calls them normally. Connections still start eagerly. Add `"directTools": ["remote_tool_name"]` inside a server entry to pin exact unprefixed names, or `["*"]` to expose that server's full schema set upfront; unknown pins are errors. Revealed schemas persist until MCP reload/restart. Exclusions and active-tool policies still apply; `mcp_search` is not an allowlist wildcard. Explicit `--tools` selections expose those schemas directly. Do not promise cost savings without measuring total usage and cache effects.
